...
# ---------- SPORTSDataIO DEPTH CHART (WITH FALLBACK) ----------
...
def fetch_depth_chart(team: str, week: int) -> Dict[str, List[str]]:
    ...
    On any HTTP or parsing error, fall back to empty lists so the app still runs.
    Team must be the SportsDataIO team key (e.g., NE, BUF).
    """
    ...
    try:
        ...
        # Fallback: engine works, but you rely on manual starter names in UI.
        return {"QB": [], "RB": [], "WR": [], "TE": []}
...
def validate_and_build_starters(
    ...
    """
    Use user starter names when provided; fall back to depth chart if missing.
    """
