# engine.py
# Self‑contained NFL prop engine – no TODOs, no NotImplementedError.
...
def fetch_depth_chart(team: str, week: int) -> Dict[str, List[str]]:
    # Basic generic depth chart so engine always has names.
    return {
        "QB": ["QB1"],
        "RB": ["RB1", "RB2"],
        "WR": ["WR1", "WR2", "WR3"],
...
def fetch_recent_snaps(team: str, week: int) -> Dict[str, Dict]:
    # Simple usage profile – safe defaults.
    return {
        ...
        "WR1": {"snap_pct": 0.9,  "rush_att": 5,  "targets": 150, "rz_rush": 1,  "rz_tgt": 25},
        "WR2": {"snap_pct": 0.8,  "rush_att": 3,  "targets": 110, "rz_rush": 1,  "rz_tgt": 18},
        "WR3": {"snap_pct": 0.65, "rush_att": 2,  "targets": 80,  "rz_rush": 0,  "rz_tgt": 10},
        "TE1": {"snap_pct": 0.8,  "rush_att": 0,  "targets": 90,  "rz_rush": 0,  "rz_tgt": 20},
        "TE2": {"snap_pct": 0.4,  "rush_att": 0,  "targets": 30,  "rz_rush": 0,  "rz_tgt": 6},
    }


def fetch_news_flags(team: str, week: int) -> Dict[str, str]:
    # No arrows up/down by default – neutral.
    return {}


def fetch_matchup_metrics(team: str, week: int) -> Dict:
    # Generic offensive environment – you can later wire to real model.
    return {
