...
import pandas as pd
from engine import project_game, fetch_rotoballer_articles
...
news_date = st.sidebar.text_input("News date (YYYY-MM-DD, optional)", value="")
...
if st.sidebar.button("Run Projections"):
        ...
        try:
            ...
            if not rows:
                ...
                front_cols = [c for c in [
                    ...
                    "passing_yards", "passing_tds", "tc_floor_passing_yards", "tc_floor_passing_tds",
                    "rushing_yards", "receiving_yards", "tc_floor_rushing_yards",
                    "tc_floor_receiving_yards", "tc_floor_total_yards",
                ...
                if news_date:
                    articles = fetch_rotoballer_articles(news_date)
                    if articles:
                        st.subheader("RotoBaller News (first 5 articles)")
                        for a in articles[:5]:
    ...
    st.info("Use the default demo (Cowboys vs Lions) and click 'Run Projections'.")